...
def make_criterion(loss, device, label_smoothing, train_loader=None, bce_pos_weight=True):
    if loss == "bce":
        pos_weight = None
...
def train_fold(model, train_loader, val_loader, epochs, lr, weight_decay,
               ...
               bce_pos_weight=True, early_stop_on="window-bacc"):
...
def main() -> None:
    ...
    for split_seed in args.split_seed:
        ...
        out_dir = output_root / f"unimodals_sngkf_{args.modal}_{args.model}_{args.channels}_sseed{split_seed}_tag{args.tag}"
        if args.modal == "aud":
            out_dir = output_root / f"unimodals_sngkf_{args.modal}_{args.model}_64mel_sseed{split_seed}_tag{args.tag}"
