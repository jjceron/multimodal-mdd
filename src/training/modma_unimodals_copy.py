...
def _window_norm(v: torch.Tensor) -> torch.Tensor:
    ...
    return (v - v.mean(dim=(1, 2), keepdim=True)) / (
        v.std(dim=(1, 2), keepdim=True) + 1e-8
    )


def build_subject_stats(dataset) -> dict[str, tuple[float, float]]:
    """Per-SUBJECT scalar (mean, std) over all that subject's windows (reference-style)."""
    stats: dict[str, tuple[float, float]] = {}
    for s in dataset.samples:
        key = "eeg" if "eeg" in s else "logmel"
        wins = s[key]
        stats[s["participant_id"]] = (float(wins.mean()), float(wins.std()) + 1e-8)
    return stats


def _norm_batch(
    x: torch.Tensor, names, subj_stats: dict[str, tuple[float, float]]
) -> torch.Tensor:
    """Apply per-subject scalar z-score to a flat window batch (each item one window)."""
    mu_vals, sig_vals = zip(*[subj_stats[n] for n in names])
    mu = torch.tensor(mu_vals, dtype=x.dtype, device=x.device)
    sig = torch.tensor(sig_vals, dtype=x.dtype, device=x.device)
    shape = [-1] + [1] * (x.dim() - 1)
    return (x - mu.view(shape)) / sig.view(shape)
...
def forward_batch(model, x, y, device, names, subj_stats, norm):
    if norm == "subj":
        x = _norm_batch(x, names, subj_stats)
    else:
        x = _window_norm(x)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for name, xb, yb in train_loader:
            logits, yf = forward_batch(
                model, xb, yb, device, names=name, subj_stats=subj_stats, norm=norm
            )
            optimizer.zero_grad()
            ...
            preds = binary_predictions(logits).cpu()
            tr_correct += int((preds == yf.cpu()).sum())
            tr_total += len(yf)
            tr_true.extend(yf.cpu().tolist())
        ...
        if early_stop_on == "subject-bacc":
            ...
            s_pred = [
                1 if (subj_logit[n] / subj_count[n]) >= 0.0 else 0 for n in subj_logit
            ]
        ...
        history["val acc"].append(float(vl_m["acc"]))
        history["train bacc"].append(float(tr_m["bacc"]))
        history["val bacc"].append(float(vl_m["bacc"]))
        logger.log_epoch(epoch, tr_loss, val_loss, tr_m, vl_m, patience_left)
        if patience_left >= patience:
            break

    if best_state is not None:
        model.load_state_dict(best_state)
    return model, max(best_epoch, 1), vl_m, history


def run_fold_test(model, test_loader, device, subj_stats=None, norm="subj") -> dict:
    ...
    with torch.no_grad():
        for names, x, y in test_loader:
            ...
            if norm == "subj":
                mu, sig = subj_stats[names[0]]
                flat = (flat - mu) / sig
            else:
                flat = _window_norm(flat)
            logits = model(flat.to(device)).cpu()
...
def train_refit(model, outer_loader, refit_epochs, args, device, subj_stats, norm):
    """Final refit: train a fresh model on ALL non-test subjects (train+val) for a
    FIXED `refit_epochs` budget. No early-stop, no best-state. Uses the same
    optimizer/loss/normalization as train_fold."""
    criterion = make_criterion(
        device, train_loader=outer_loader, pos_weight=args.pos_weight
    )
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=args.lr, weight_decay=args.weight_decay, foreach=False
    )
    model.train()
    for _ in range(max(refit_epochs, 1)):
        for name, xb, yb in outer_loader:
            logits, yf = forward_batch(
                model, xb, yb, device, names=name, subj_stats=subj_stats, norm=norm
            )
            optimizer.zero_grad()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Unimodal classification for EEG and Audio MODMA modalities"
    )
    parser.add_argument("--modal", type=str, default="eeg", choices=["eeg", "aud"])
    parser.add_argument(
        "--model", type=str, default="deepconvnet", choices=sorted(MODEL_CLASSES)
    )
    parser.add_argument(
        "--channels", type=str, default="f64", choices=["all", "10-20", "f64", "29"]
    )
    ...
    parser.add_argument(
        ...
        default="subj",
        choices=["subj", "window"],
        help="Input normalization: per-subject (subj) or per-window (window)",
    )
    parser.add_argument(
        "--early-stop-on",
        type=str,
        default="subject-bacc",
        choices=["window-bacc", "subject-bacc"],
    )
    parser.add_argument(
        "--pos-weight",
        type=float,
    ...
    parser.add_argument(
        "--save-model",
        action="store_true",
        help="Save the final model per-fold model checkpoint '.pt'",
    )
...
def main() -> None:
    ...
    output_root = (
        ...
        if args.output_root
        else (PROJECT_ROOT / "outputs/results/unimodals" / args.modal)
    )

    subj_stats = build_subject_stats(dataset) if args.norm == "subj" else None
    ...
    for split_seed in args.split_seed:
        ...
        print("=" * 70)
        print(
            f" UNIMODAL | modal={args.modal} model={args.model} channels={args.channels} "
            f"subjects={len(dataset.samples)} n_ch={n_channels} n_samples={n_samples}"
        )
        print(
            f" device={device} gpu={gpu} params(total={total}, trainable={trainable})"
        )
        print("=" * 70)
        ...
        for fold_index, (
            ...
            print(f"\n=== FOLD {fold_index} ===")
            # Initial train/val pass: only for validation curves + info.
            model = build_model(
                args.model,
                n_channels,
                n_classes=n_classes,
            ...
            model, best_epoch, vl_m, fold_history = train_fold(
                model,
                train_loader,
                val_loader,
                args.epochs,
            ...
            results["training curves"][f"fold {fold_index}"] = fold_history
            del model
            torch.cuda.empty_cache()

            # Refit on ALL non-test subjects
            model = build_model(
                args.model,
                n_channels,
                n_classes=n_classes,
            ...
            ref_epochs = max(best_epoch, 15)
            print(f"  === REFIT (train+val, epochs={ref_epochs})")
            model = train_refit(
                model,
                outer_loader,
                ref_epochs,
            ...
            fres = run_fold_test(
                model, test_loader, device, subj_stats=subj_stats, norm=args.norm
            )
            fold_res = {
                ...
                "n_train": len(
                    set(train_loader.dataset.names) | set(val_loader.dataset.names)
                ),
                "n_test": len(set(test_loader.dataset.names)),
                "test_subjects": sorted(set(test_loader.dataset.names)),
                "val_metrics": {
                    k: float(vl_m[k]) for k in ("bacc", "f1", "sens", "spec")
                },
                "best_epoch": max(best_epoch, 1),
                "refit_epochs": ref_epochs,
            }
