...
* The validated signal lives in *band-power/topography/dynamics/entropy*
  ...
  1. **SpectralConvNet (CNN branch)** — a light, EEGNet-style depthwise
     ...
     per filter it pools log band-energy over time, giving a per-window
     learned-band descriptor. It has ~6k params and is trained on
...
class SpectralConvNet(nn.Module):
    """Per-window learned spectral encoder (single depthwise branch).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        energy = torch.log1p(h.pow(2).mean(dim=2))  # [B, n_filters]
